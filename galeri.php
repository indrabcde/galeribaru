  ...
  <table class="table  table-bordered table-hover " style="width : 720px justify-center">
    <thead class="table" style="text-align: center;">
      ...
      while ($data = mysqli_fetch_array($sql)) {
        ...
        $userHasLiked = mysqli_num_rows($sqlCheckLike) > 0;
    ...
    <tbody>
      <tr>
        ...
        <td>
          <?php if ($userHasLiked) { ?>
            <a href="unlike.php?fotoid=<?= $data['fotoid'] ?>" class="btn btn-danger">Batal suka</a>
          <?php } else { ?>
            <a href="like.php?fotoid=<?= $data['fotoid'] ?>" class="btn btn-outline-danger">Suka</a>
          <?php } ?>


          <a href="komentar.php?fotoid=<?= $data['fotoid'] ?>" class="btn btn-outline-dark">Komentar</a>
        </td>
      </tr>
    </tbody>
  <?php
